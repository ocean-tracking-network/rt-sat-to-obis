...
def prompt_potential_match_otn_tags(engine: Engine, deployment_df: pd.DataFrame, collectioncode: str='unknown'):
    ...
    if collectioncode and collectioncode.lower() !='unknown':
        ...
        if not subset_otn_sat_tag_df.empty:
...
def do_match(engine: Engine, widget: Any, tag_ref: str, submit_btn: Button):
    ...
    if widget.value not in all_otn_sat_tags:
        print(f'catalognumber: {widget.value} is not found in {TAG_REF_CATALOGNUMBER_MATCH_TABLE}. Please try again.')
        widget.value = f'{widget.value} is not found - try again'
        return
